...
export const BlockMetadata: MessageFns<BlockMetadata, "cosmos.store.v1beta1.BlockMetadata"> = {
  ...
  encode(message: BlockMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.response_commit !== undefined) {
      ResponseCommit.encode(message.response_commit, writer.uint32(50).fork()).join();
    }
    if (message.request_finalize_block !== undefined) {
      RequestFinalizeBlock.encode(message.request_finalize_block, writer.uint32(58).fork()).join();
    }
    if (message.response_finalize_block !== undefined) {
      ResponseFinalizeBlock.encode(message.response_finalize_block, writer.uint32(66).fork()).join();
    }
    return writer;
  },
