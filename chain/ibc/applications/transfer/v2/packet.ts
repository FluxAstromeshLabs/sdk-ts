...
export const FungibleTokenPacketDataV2: MessageFns<
...
> = {
  ...
  encode(message: FungibleTokenPacketDataV2, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    ...
    if (message.forwarding !== undefined) {
      ForwardingPacketData.encode(message.forwarding, writer.uint32(42).fork()).join();
    }
    return writer;
  },
...
export const ForwardingPacketData: MessageFns<
...
> = {
  ...
  encode(message: ForwardingPacketData, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.destination_memo !== "") {
      writer.uint32(10).string(message.destination_memo);
    }
    for (const v of message.hops) {
      Hop.encode(v!, writer.uint32(18).fork()).join();
    }
    return writer;
  },
