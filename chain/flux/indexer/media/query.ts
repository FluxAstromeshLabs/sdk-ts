...
export const S3Obj: MessageFns<S3Obj, "flux.indexer.media.S3Obj"> = {
  $type: "flux.indexer.media.S3Obj" as const,

  encode(message: S3Obj, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.op !== 0) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): S3Obj {
...
export const PresignedURLRequest: MessageFns<PresignedURLRequest, "flux.indexer.media.PresignedURLRequest"> = {
  $type: "flux.indexer.media.PresignedURLRequest" as const,

  encode(message: PresignedURLRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.path !== "") {
      writer.uint32(10).string(message.path);
    }
    for (const v of message.objs) {
      S3Obj.encode(v!, writer.uint32(18).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): PresignedURLRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const PresignedURLResponse: MessageFns<PresignedURLResponse, "flux.indexer.media.PresignedURLResponse"> = {
