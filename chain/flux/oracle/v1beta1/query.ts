...
export const QuerySimpleEntry: MessageFns<QuerySimpleEntry, "flux.oracle.v1beta1.QuerySimpleEntry"> = {
  $type: "flux.oracle.v1beta1.QuerySimpleEntry" as const,

  encode(message: QuerySimpleEntry, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.symbols !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): QuerySimpleEntry {
...
export const QuerySimpleEntryResponse: MessageFns<
  QuerySimpleEntryResponse,
  "flux.oracle.v1beta1.QuerySimpleEntryResponse"
> = {
  $type: "flux.oracle.v1beta1.QuerySimpleEntryResponse" as const,

  encode(message: QuerySimpleEntryResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.entries) {
      SimpleEntry.encode(v!, writer.uint32(10).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): QuerySimpleEntryResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
