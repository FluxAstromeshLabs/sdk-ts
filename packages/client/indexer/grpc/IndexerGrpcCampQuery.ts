...
export class IndexerGrpcCampQuery extends BaseIndexerGrpc {
  ...
  async getMissions(
    request: Partial<campclashQuery.GetMissionsRequest>,
  ): Promise<campclashQuery.GetMissionsResponse> {
    let response = await this.retry(() => this.client.GetMissions(request))
    return response as campclashQuery.GetMissionsResponse
  }
  async checkinDailyLogin(
    request: Partial<campclashQuery.CheckinDailyLoginRequest>,
  ): Promise<campclashQuery.CheckinDailyLoginResponse> {
